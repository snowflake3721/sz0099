 ...
 * </pre>
 */
public class CoeArticleConverter {
	...
	public CircleMain convert(List<CoeArticle> contentList , CoeUser currentLoginUser) {
	...
	public CircleMain convert(List<CoeArticle> contentList, CircleMain circleMain){
		if(circleMain==null) {
		...
		if(null != contentList && !contentList.isEmpty()) {
			int i=0;
			for(CoeArticle content : contentList) {
				if(i==0) {
				...
				List<CoeArticleTag> tagList = content.getArticleTagList();
				if(null != tagList && !tagList.isEmpty()) {
					CoeArticleTag tag = tagList.get(0);
	...
	public List<CircleItem> initItemForHuan(Long userId) {
		...
		if(null != page && page.getTotalElements()>0) {
			...
			//没有绑定人（即没有推广者），查询位置下的技能推广列表，取一个
			 Page<CoeArticle> coeArticlePage = coeArticleWrapper.findPageForRandomUserId(userId);
			 if(null != coeArticlePage && coeArticlePage.getTotalElements()>0) {
				 List<CoeArticle> content = coeArticlePage.getContent();
					if(null != content && !content.isEmpty()) {
						itemList = new ArrayList<>(content.size());
						for(CoeArticle p : content) {
	...
	public CircleItem initItemForJian(Long userId) {
		String jian="#";
		CoeArticlePosition recommendPosition = new CoeArticlePosition();
		...
		CoeArticlePageDto recommendDto = coeArticleFetcher.fetchRecommendForMainCircle(recommendPosition, CoeArticleFetcher.STRATEGY_DEFAULT);
		
		 Page<CoeCategArticle> categoryPage=recommendDto.getCategoryPage();
		 Page<CoeArticlePosition> positionPage=recommendDto.getPositionPage();
		 if(null != positionPage && positionPage.getTotalElements()>0) {
			 CoeArticlePosition article = positionPage.getContent().get(0);
			...
			//获取最新一篇 分类为[技能]文章
			 CoeCategArticle category = categoryPage.getContent().get(0);
			 CoeArticle article = category.getArticle();
			 jian=LinkUtil.getLink(article.getLink(), article.getOriginalLink(), jian);
		 }else {
			 if(null != userId) {
				 //获取主推
	...
	public CoeArticleWrapper getCoeArticleWrapper() {
		return coeArticleWrapper;
	}

	public void setCoeArticleWrapper(CoeArticleWrapper coeArticleWrapper) {
		this.coeArticleWrapper = coeArticleWrapper;
	}
