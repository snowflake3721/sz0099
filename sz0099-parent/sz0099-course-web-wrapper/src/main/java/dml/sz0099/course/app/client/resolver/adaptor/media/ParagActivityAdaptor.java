...
public class ParagActivityAdaptor extends ImageDefaultAdaptor<Paragraph>{
	...
	public Paragraph convert(ImageExtend imageExtend) {
		...
		if(null != imageExtend) {
			...
			ParagActivity activity = new ParagActivity();
			paragraph.setActivity(activity);
			...
			if(null != images && !images.isEmpty()) {
				...
				for(Imagebase image : images) {
					...
					photoParag.setCreatedBy(ref.getCreatedBy());
					photoParag.setLastModifiedBy(ref.getLastModifiedBy());
					if(activity.getParagId()==null) {
						activity.setParagId(ref.getSubId());
						activity.setMainId(ref.getMainId());
					}
					photoParag.setUserId(ref.getUserId());
	...
	public boolean deleteFromRemote(ParagActivity paragActivity) {
		
		Long paragId = paragActivity.getParagId();
		Long productId = paragActivity.getMainId();
		...
		imageRequest.setUserId(paragActivity.getUserId());
