...
import dml.sz0099.course.app.biz.delegate.article.ArticlePhotoCoverDelegate;
 ...
 * </pre>
 */
@Service("articlePhotoCoverWrapperImpl")
public class PhotoCoverWrapperImpl implements PhotoCoverWrapper {
	...
	@Autowired
	private ArticlePhotoCoverDelegate photoCoverDelegate;
	...
	public PhotoCover fillAndConvert(ImageRef ref, PhotoCover entity) {
		...
		if(null != photo) {
			...
			String accessUrl = ImagePathUtil.buildAccessUrl(coverArticleAdaptor.getAccessUrlMapping(),fullnameWithRelative);
	...
	public PhotoCover createPhotoCover(ImageRef ref) {
		...
		base.setStrategy(1);//设置单一生成策略
		String basePath = coverArticleAdaptor.getBasePath();
		String basePathFolder = coverArticleAdaptor.getBasePathFolder();
		String accessUrlMapping = coverArticleAdaptor.getAccessUrlMapping();
		ImageExtend extend = coverArticleAdaptor.config();
		ref = ImagePathUtil.fillRefThenGenerate(extend, ref, basePath, basePathFolder, accessUrlMapping);
		PhotoCover photoCover = coverArticleAdaptor.convertFromRef(ref);
