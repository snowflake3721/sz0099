...
import dml.sz0099.course.app.biz.delegate.activity.ActivityPhotoBannerDelegate;
 ...
 * </pre>
 */
@Service("activityPhotoBannerWrapperImpl")
public class PhotoBannerWrapperImpl implements PhotoBannerWrapper {
	...
	@Autowired
	private ActivityPhotoBannerDelegate photoBannerDelegate;
	...
	public PhotoBanner fillAndConvert(ImageRef ref, PhotoBanner entity) {
		...
		if(null != photo) {
			...
			String accessUrl = ImagePathUtil.buildAccessUrl(bannerActivityAdaptor.getAccessUrlMapping(),fullnameWithRelative);
	...
	public PhotoBanner createPhotoBanner(ImageRef ref) {
		...
		base.setStrategy(1);//设置单一生成策略
		String basePath = bannerActivityAdaptor.getBasePath();
		String basePathFolder = bannerActivityAdaptor.getBasePathFolder();
		String accessUrlMapping = bannerActivityAdaptor.getAccessUrlMapping();
		...
		PhotoBanner photoBanner = bannerActivityAdaptor.convertFromRef(ref);
