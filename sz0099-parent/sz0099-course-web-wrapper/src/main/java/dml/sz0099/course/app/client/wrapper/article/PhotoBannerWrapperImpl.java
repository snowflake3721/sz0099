...
import dml.sz0099.course.app.biz.delegate.article.ArticlePhotoBannerDelegate;
 ...
 * </pre>
 */
@Service("articlePhotoBannerWrapperImpl")
public class PhotoBannerWrapperImpl implements PhotoBannerWrapper {
	...
	@Autowired
	private ArticlePhotoBannerDelegate photoBannerDelegate;
	...
	public PhotoBanner fillAndConvert(ImageRef ref, PhotoBanner entity) {
		...
		if(null != photo) {
			...
			String accessUrl = ImagePathUtil.buildAccessUrl(bannerArticleAdaptor.getAccessUrlMapping(),fullnameWithRelative);
	...
	public PhotoBanner createPhotoBanner(ImageRef ref) {
		...
		base.setStrategy(1);//设置单一生成策略
		String basePath = bannerArticleAdaptor.getBasePath();
		String basePathFolder = bannerArticleAdaptor.getBasePathFolder();
		String accessUrlMapping = bannerArticleAdaptor.getAccessUrlMapping();
		...
		PhotoBanner photoBanner = bannerArticleAdaptor.convertFromRef(ref);
