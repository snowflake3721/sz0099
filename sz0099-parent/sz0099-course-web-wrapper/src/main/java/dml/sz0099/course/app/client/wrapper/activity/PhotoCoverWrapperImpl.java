...
import dml.sz0099.course.app.biz.delegate.activity.ActivityPhotoCoverDelegate;
 ...
 * </pre>
 */
@Service("activityPhotoCoverWrapperImpl")
public class PhotoCoverWrapperImpl implements PhotoCoverWrapper {
	...
	@Autowired
	private ActivityPhotoCoverDelegate photoCoverDelegate;
	...
	public PhotoCover fillAndConvert(ImageRef ref, PhotoCover entity) {
		...
		if(null != photo) {
			...
			String accessUrl = ImagePathUtil.buildAccessUrl(coverActivityAdaptor.getAccessUrlMapping(),fullnameWithRelative);
			...
			//ref = coverActivityAdaptor.convertToRef(ref,entity);
			base.setFile(dest);
	...
	public PhotoCover createPhotoCover(ImageRef ref) {
		...
		base.setStrategy(1);//设置单一生成策略
		String basePath = coverActivityAdaptor.getBasePath();
		String basePathFolder = coverActivityAdaptor.getBasePathFolder();
		String accessUrlMapping = coverActivityAdaptor.getAccessUrlMapping();
		ImageExtend extend = coverActivityAdaptor.config();
		ref = ImagePathUtil.fillRefThenGenerate(extend, ref, basePath, basePathFolder, accessUrlMapping);
		PhotoCover photoCover = coverActivityAdaptor.convertFromRef(ref);
