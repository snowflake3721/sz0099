...
import dml.sz0099.course.app.biz.delegate.profession.ProfPhotoParagDelegate;
 ...
 * </pre>
 */
@Service("profPhotoParagWrapperImpl")
public class PhotoParagWrapperImpl implements PhotoParagWrapper {
	...
	@Autowired
	private ProfPhotoParagDelegate photoParagDelegate;
	
	@Autowired
	private ParagProfessionAdaptor paragProfessionAdaptor;
	...
	public PhotoParag fillAndConvert(ImageRef ref, PhotoParag entity) {
		...
		if(null != photo) {
			...
			String accessUrl = ImagePathUtil.buildAccessUrl(paragProfessionAdaptor.getAccessUrlMapping(),fullnameWithRelative);
	...
	public PhotoParag createPhotoParag(ImageRef ref) {
		...
		base.setStrategy(1);//设置单一生成策略
		String basePath = paragProfessionAdaptor.getBasePath();
		String basePathFolder = paragProfessionAdaptor.getBasePathFolder();
		String accessUrlMapping = paragProfessionAdaptor.getAccessUrlMapping();
		ImageExtend extend = paragProfessionAdaptor.config();
		ref = ImagePathUtil.fillRefThenGenerate(extend, ref, basePath, basePathFolder, accessUrlMapping);
		PhotoParag photoParag = paragProfessionAdaptor.convertFromRef(ref);
