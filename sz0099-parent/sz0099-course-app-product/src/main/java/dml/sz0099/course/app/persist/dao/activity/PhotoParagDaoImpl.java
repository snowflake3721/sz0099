...
import dml.sz0099.course.app.persist.repository.activity.PhotoParagRepository;
import dml.sz0099.course.app.persist.specification.activity.PhotoParagSpecification;
 ...
 * 
 */
@Repository("activityPhotoParagDaoImpl")
public class PhotoParagDaoImpl extends GenericDaoImpl<PhotoParag, Long> implements PhotoParagDao, Serializable {
	...
	public void mergeTemplate(Long mainId, Integer template) {
		photoParagRepository.mergeTemplate(mainId, template);
	}
	public void mergeTemplatePart(Long mainId, Integer template) {
		photoParagRepository.mergeTemplatePart(mainId, template);
	}
	
	public void mergeTemplateById(Long id, Integer template) {
		photoParagRepository.mergeTemplateById(id, template);
	}
	
	public List<PhotoParag> findTemplateByMainId(Long mainId){
		List<PhotoParag> entityList = photoParagRepository.findTemplateByMainId(mainId);
		return entityList;
	}
	
	public List<PhotoParag> findByMainId(Long mainId){
		List<PhotoParag> entityList = photoParagRepository.findByMainId(mainId);
		return entityList;
	}
	
	public Long countNotSelfByTemplateId(Long templateId) {
		Long num = photoParagRepository.countNotSelfByTemplateId(templateId);
		return num;
	}
}
