...
import dml.sz0099.course.app.biz.service.article.PhotoBannerService;
import dml.sz0099.course.app.persist.entity.article.PhotoBanner;
import dml.sz0099.course.app.persist.entity.article.PhotoBanner;
import dml.sz0099.course.app.persist.entity.article.bo.PhotoBannerBo;
 ...
 */
@Service
public class ArticlePhotoBannerDelegateImpl implements ArticlePhotoBannerDelegate, Serializable {

	private static final Logger LOGGER = LoggerFactory.getLogger(ArticlePhotoBannerDelegateImpl.class);
	...
	public PhotoBanner findById(Long id, boolean withPhoto) {
		return photoBannerService.findById(id,withPhoto);
	}
	...
	public void deleteImageById(PhotoBanner photoBanner, boolean cascade) {
		photoBannerService.deleteImageById(photoBanner, cascade);
	}
	
	public PhotoBanner addImageById(PhotoBanner photoBanner, boolean cascade) {
		return photoBannerService.addImageById(photoBanner, cascade);
	}

	public PhotoBanner mergeRefForDescription(PhotoBanner photoBanner) {
		
		return photoBannerService.mergeRefForDescription(photoBanner);
	}
	public PhotoBanner mergeRefForOrder(PhotoBanner photoBanner) {
		
		return photoBannerService.mergeRefForOrder(photoBanner);
	}
	
	public Long countByMainId(Long mainId) {
		return photoBannerService.countByMainId(mainId);
	}
}
