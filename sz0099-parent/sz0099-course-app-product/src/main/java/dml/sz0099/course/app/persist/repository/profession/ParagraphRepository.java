...
@Repository("profParagraphRepository")
public interface ParagraphRepository extends BasicJpaRepository<Paragraph,Long> {

	@Query("select e from dml.sz0099.course.app.persist.entity.profession.Paragraph e where e.id=?1")
	Paragraph findById(Long id);
	
	@Query("select e from dml.sz0099.course.app.persist.entity.profession.Paragraph e where e.aid=?1")
	Paragraph findByAid(Long aid);
	
	@Query("select e from dml.sz0099.course.app.persist.entity.profession.Paragraph e where e.id in ?1 order by e.orderSeq asc")
	public List<Paragraph> findByIdList(List<Long> idList);
	
	@CodeGenerated
	@Modifying
	@Query("delete from dml.sz0099.course.app.persist.entity.profession.Paragraph e where e.id in ?1 and e.userId=?2")
