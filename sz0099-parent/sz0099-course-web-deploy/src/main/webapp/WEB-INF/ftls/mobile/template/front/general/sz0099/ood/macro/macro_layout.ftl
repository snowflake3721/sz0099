...
<#-- 单篇封面图滚 -->
<#macro Mg_layout_imgCoverRollerH_single entity0=null idPre="layout_" withPreIntro=false withTitle=true withSubTitle=false showLoadMore=false showDot=false showPreNext=false showPenname=false linkable=false style="height:78px">
<#if entity0??>
...
<div id="id_roller_${idPre}_${entity0.id}" class="carousel slide" data-ride="carousel">
    ...
    <#if coverList!  && coverList?size &gt; 0 >
          <#list coverList as banner>
          	  ...
          	  <#if bannerImage! >
	          <div class="item <#if banner?index==0>active</#if>" <#if style!>style="${style}"</#if>>
	          	<#-- onclick="showBigView('${banner.fullurl}',720, ${banner.width},'${banner.id}')" -->
	            	<a href="<#if linkable>${getLink(entity0, "/")}<#else>javascript:void(0)</#if>" class="thumbnail img-responsive" alt="${entity0.title}" target="_self">
	            	<img data-src="holder.js/144x108/auto/#777:#777"  height="108px" src="${bannerImage}" title="${entity0.title}" data-holder-rendered="true">
	          		<#if withPreIntro && withTitle && withSubTitle>
		          		<div class="carousel-caption">
		          			<#if withPreIntro && entity0.preIntro!>
				        	<p class="text-left"><small>${entity0.preIntro}</small></p>
				      		</#if>
		          			<#if withTitle && entity0.title!>
				        	<h4>${entity0.title}</h4>
				      		</#if>
		          			<#if withSubTitle && entity0.subTitle!>
				        	<p class="text-right"><small>${entity0.subTitle}</small></p>
				      		</#if>
				      	</div>
			      	</#if>
			      	</img>
			      	<#if showPenname && entity0.penname!>
	          		<div class="container">
...
<#if coverImage! >
<div class="item active" style="height:108px">
<#-- onclick="showBigView('${coverImage}',720, 720,'${entity0.id}')" -->
		<a href='<#if linkable>${getLink(entity0, "/")}<#else>javascript:void(0)</#if>' class="thumbnail" >
		  <img id='id_tpl_media_common_img${image.id}' class="img-responsive"  height='108px' src='${coverImage}' alt='${entity0.title}'>
		  <div class="carousel-caption">
	  			<#if withPreIntro>
	        	<p class="text-left"><small>${entity0.preIntro}</small></p>
	      		</#if>
...
<#-- 单篇封面图滚 -->
<#macro Mg_layout_imgCoverRollerH_single2 entity0=null idPre="layout_" withPreIntro=false withTitle=true withSubTitle=false showLoadMore=false showDot=false showPreNext=false showPenname=false linkable=false >
<#if entity0??>
<#assign coverList=entity0.coverList/>
<#if coverList! && coverList?size &gt; 0>
<div id="id_roller_${idPre}_${entity0.id}" class="carousel slide" data-ride="carousel">
    ...
    <#if coverList!  && coverList?size &gt; 0 >
          <#list coverList as banner>
          	  <#assign bannerImage=PhotoUtil.getShowUrlForExpected2(banner.fullurl,300, banner.width) />
          	  <#if bannerImage! >
	          <div class="item <#if banner?index==0>active</#if>" style="height:108px">
	          <#-- onclick="showBigView('${banner.fullurl}',720, ${banner.width},'${banner.id}')" -->
...
<div class="item active" style="height:108px">
		<#-- onclick="showBigView('${coverImage}',720, 720,'${entity0.id}')" -->
		<a href='<#if linkable>${getLink(entity0, "/")}<#else>javascript:void(0)</#if>' class="thumbnail " style="height:108px">
		  <img id='id_tpl_media_common_img${image.id}'  class="img-responsive" height='108px' src='${coverImage}' alt='${entity0.title}'>
		  <div class="carousel-caption">
	  			<#if withPreIntro>
	        	<p class="text-left"><small>${entity0.preIntro}</small></p>
			  ...
			  <div class="panel-body" sytle="margin-bottom:0px">
				  	<div class="row">
						  <div class="col-xs-6 col-md-4">
						    ...
						    <p>${getSubstring(entity0.description,20,'暂无')}</p>
						    
						  </div>
						  <div class="col-xs-6 col-md-4">
						    	<@Mg_layout_imgCoverRollerH_single entity0=entity0 idPre="layout_list" withPreIntro=false withTitle=false withSubTitle=false showLoadMore=false showDot=false showPreNext=false/>
						  </div>
					</div>
			  </div><#-- end body -->
		 </div>
	          ...
	          <div class="item <#if entity0?index==0>active</#if>">
	            	...
	            	<#if showPenname>
							  ...
							  <#if entity0.proTagList!?size gt 0 > 
							  	<@M_showTagListRandom productTagList=entity0.proTagList idPre="tag_list_" entity0=entity0 fontSize=12 randomSize=false withWrapper=false/>
							  	</#if>
			  					</strong>
			  					</span>
