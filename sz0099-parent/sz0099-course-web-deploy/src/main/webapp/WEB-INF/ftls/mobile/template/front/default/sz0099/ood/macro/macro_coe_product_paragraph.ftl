...
<a class="btn btn-xs btn-primary" href="javascript:void(0)" onclick="addParagraphSingle('${id}','/sz0099/ood/product/manage/paragraph/add')">添加段落</a>
<a class="btn btn-xs btn-warning" href="javascript:void(0)" onclick="refreshParagraph('${id}','/sz0099/ood/product/manage/paragraph/editListUI')">刷新段落</a>
<a class="btn btn-xs btn-danger" href="javascript:void(0)" onclick="deleteAllParagraph('${id}','/sz0099/ood/product/manage/paragraph/deleteAll')">全部删除</a>
</div>
<br/>
</#macro>
...
<div id="paragragh_${entity.id}"> <!--div wrapper-->
	...
	<div class="panel panel-danger" id="id_panel_paragId${entity.paragId}" name="paragraph">
		  <div class="panel-heading">
		    	...
		    	<button type="button" class="btn btn-xs btn-primary pull-left" onclick="saveParagraphSingle('${entity.paragId}','/sz0099/ood/product/manage/paragraph/mergeSingle')">保存</button>
		    	...
		    	<button type="button" class="btn btn-xs btn-danger pull-right" onclick="deleteParagraphSingle('${entity.paragId}', '/sz0099/ood/product/manage/paragraph/deleteSingle')">删除</button>
		  ...
		  <div class="panel-body">
			  ...
			  <div class="form-group form-group-sm">
			    <label for="id_parag_title${entity.paragId}">段落标题(少于60字符)</label>
			  ...
			  <div class="form-group form-group-sm">
			    <label for="id_parag_name${entity.paragId}"><small>段落右角标(简短，少于20字符)</small></label>
			    <input type="text" id="id_parag_name${entity.paragId}" 
			    name="name" 
			    onchange="onChangeShow('id_parag_name_show${entity.paragId}',this.value,10)"
			    value="${entity.paragraph.name}" class="form-control" placeholder="段落右角标">
			  </div>
			  
				<div class="file-loading">
					<input id="id_file_${entity.paragId}" data-devId="sz0099" data-project="ood" data-module="product" data-variety="product" data-strategy="1" data-position="paragragh" data-mainId="${entity.mainId}" data-subId="${entity.paragId}" type="file" multiple name="files" />
				</div>
				<p class="text-danger">
					<small>
		    ...
		    <div class="panel-footer">
		    	<p class="text-center">每段都要点保存哦！-->
		    	<button type="button" class="btn btn-sm btn-primary pull-right" onclick="saveParagraphSingle('${entity.paragId}','/sz0099/ood/product/manage/paragraph/mergeSingle')">保存当前段落</button>
		    	</p>
		    </div>
	  </div><!--end panel-->
</div><!--end div wrapper-->
...
<div class='media' id='id_tpl_media_id${photoParag.id}'>
  <div class='media-left'>
	<a href='javascript:void(0)' onclick="showBigView('${photoParag.fullurl}',800,'${photoParag.width}','${photoParag.id}')">
	  <img id='id_tpl_media_common_img${photoParag.id}' class='media-object' width='200px' src='${PhotoUtil.getShowUrlForAdForParag(photoParag)}' alt='${photoParag.id}'>
	</a>
  </div> 
...
<div id="paragragh_view_${entity.id}"> <!--div wrapper-->
			   ...
			   <h5 class="text-center"><mark>${paragraph.title}</mark></h5>
			   <p class="text-left" >
		...
		<#if photoParag!>
				<li>
			    	<a href='javascript:void(0)' onclick="showBigView('${photoParag.fullurl}',1024,'${photoParag.width}','${photoParag.aid}')">
					  <img id='id_tpl_media_common_img${photoParag.aid}' class='img-responsive' src='${PhotoUtil.getShowUrlForExpected(photoParag,720)}' alt='${photoParag.title}'>
					</a>
					<p>${photoParag.title}</p>
		    	</li>
		</#if>
		</#list>
		</ul>
