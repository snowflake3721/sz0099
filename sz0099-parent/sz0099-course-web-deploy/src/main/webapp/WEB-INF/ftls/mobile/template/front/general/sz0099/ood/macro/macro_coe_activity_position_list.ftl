...
<#-- 选择活动 -->
<#macro M_selectActivityPositionPage page positionRef=null moreLoaded="true" url="/sz0099/ood/activity/manage/queryActivityManageList" condition="">   
<#if page??>
     ...
     <div class="btn-group btn-group-xs btn-group-justified" role="group" aria-label="id_art${item.id}">
	      <a id="id_art${positionRef.baseId}_${entity.id}" class="btn btn-default btn-xs"  role="button" data-toggle="collapse" href="#id_art_content${positionRef.baseId}_${entity.id}" aria-expanded="true" aria-controls="id_art_content${positionRef.baseId}_${entity.id}">
						 ...
						 <span class="glyphicon glyphicon-list" aria-hidden="false"></span>
						  ${entity.activityNo} -- ${getSubstring(entity.title,12)} <span class="pull-right"> <strong>#${item?index}</strong></span>
...
<#macro M_activityPositionPageFlag positon=null entityPage=null flag="QX" tip="插旗行动" >
...
<#if entityPage! && entityPage.totalElements gt 0 >
	...
	<div class="jumbotron">
	  ...
	  <a class="btn btn-danger btn-lg" href="/sz0099/ood/home/activity/index/recommend?st=general" role="button">进入圈里</a>
	  </p>
	</div>
	 -->
