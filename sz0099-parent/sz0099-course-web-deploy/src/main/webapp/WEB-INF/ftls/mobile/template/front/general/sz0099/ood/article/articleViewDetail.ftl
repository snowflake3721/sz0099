...
<@M_navBreadForCategory category="qunxia" subCategory="article" showView="detail" entity=entity/>
<#if page! && entity!>
  ...
  <@Mg_coeArticleDetailView entity=entity paragraphPage=page moreLoaded="true" />
</#if>

<@M_navBottomForCategory category="qunxia" subCategory="article" showView="detail"  entity=entity/>
<@shiro.user>
<#--我的邀请二维码 	-->
<@M_qrCodeViewForIndex user=UserUtils.getUser()/>
<#assign description=getSubstring(entity.description,30,' -- 群侠')/>
<#assign imgUrl='/assets/common/tools/blooming-menu3/img/qunxia_6_2.jpg'/>
<#assign coverImageList=entity.coverList />
<#if coverImageList!?size &gt; 0 >      
  	<#assign coverImage=coverImageList?first/>
  	<#if coverImage! && coverImage.expectedUrl!> 
  	<#assign imgUrl=coverImage.expectedUrl/>
	</#if>
	<#else>
...
<input type="hidden" value="${login_status}" name="login_status" id="id_login_status"
	...
	data-desc="${description} 【群侠】"
/>
</@shiro.user>
</div>

<div class="container" id="body_footer_js">	
<script>document.write("<script type='text/javascript' src='/assets/theme/general/sz0099/ood/article/manage/coeArticle_manage.js?v=1.0.0'><\/script>");</script>
