    ...
    <#if articleList!?size &gt; 0 >
          <#list articleList as article>
	          <div class="item <#if article?index==0>active</#if>">
	            	...
	            	<img data-src="holder.js/720x540/auto/#777:#777" alt="${article.title}" height="540px" src="${article.firstBannerImage}" title="${article.title}" data-holder-rendered="true">
	          		<div class="carousel-caption">
			        	<h3>${article.title}</h3>
	          			<#if withTitle=="true">
			        		<p>${article.subTitle}</p>
...
<#macro M_bannerImageRollerForView imageRollerId article withTitle="false">
<!--单篇文章：文章banner的图片滚动，开始-->
<div id="carousel-image-roller-${imageRollerId}" class="carousel slide" data-ride="carousel">
    <#if article??>
    <ol class="carousel-indicators">
    	  <#assign bannerImageList=article.bannerImageList />
		    <#if bannerImageList!?size &gt; 0 >      
		          <#list bannerImageList as bannerImage>
		           <li data-target="#carousel-image-roller-${imageRollerId}"  data-slide-to="${bannerImage?index}" class="<#if bannerImage?index==0>active</#if>" id="image_roller_${article.id}_${bannerImage?index}"></li>
		           <#else>
		           <li data-target="#carousel-image-roller-${imageRollerId}"  data-slide-to="0" class="" id="${imageRollerId}_no_item">正在准备中......</li>
		          </#list>
		    </#if>
    ...
    <div class="carousel-inner" role="listbox">
	    <#if bannerImageList!?size &gt; 0 >
	          <#list bannerImageList as bannerImage>
		          <div class="item <#if bannerImage?index==0>active</#if>">
		            	<img data-src="holder.js/720x540/auto/#777:#777" alt="${article.title}_${bannerImage?index}" src="${bannerImage.bannerUrl}" title="${article.title}" data-holder-rendered="true">
		          		<#if withTitle=="true">
		          		<div class="carousel-caption">
				        	<#-- <h3>${article.title}</h3> -->
				        	<p>${bannerImage.thirdTitle}</p>
				      	</div>
				      	</#if>
		          </div>
		          <#else>
...
<#macro MV_bannerImageRollerForView imageRollerId articleName="articleView" withTitle="false">
<!--单篇文章：文章banner的图片滚动，开始-->
<div id="carousel-image-roller-${imageRollerId}" class="carousel slide" data-ride="carousel">
    <ol class="carousel-indicators">
		           <li v-show="articleView.bannerImageList.length>0" data-target="#carousel-image-roller-${imageRollerId}" v-for="(bannerImage,index) in articleView.bannerImageList"  :data-slide-to="index" v-bind:class="index==0?'active':''" :id="'image_roller_' + bannerImage.id + '_' + index"></li>
    </ol>
    <div class="carousel-inner" role="listbox">
		          <div v-bind:class="index==0?'item active':'item'" v-for="(bannerImage,index) in articleView.bannerImageList" v-show="articleView.bannerImageList.length>0" >
		            	
		            	<img data-src="holder.js/720x540/auto/#777:#777" :alt="articleView.title+'_'+index" :src="bannerImage.articlePhotoItem.accessUrl" :title="bannerImage.thirdTitle" data-holder-rendered="true">
