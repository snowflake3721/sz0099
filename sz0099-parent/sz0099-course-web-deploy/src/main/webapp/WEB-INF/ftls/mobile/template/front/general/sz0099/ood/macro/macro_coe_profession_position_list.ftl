...
<#-- 选择文章 -->
<#macro M_selectProfessionPositionPage page positionRef=null moreLoaded="true" url="/sz0099/ood/profession/manage/queryProfessionManageList" condition="">   
<#if page??>
     ...
     <div class="btn-group btn-group-xs btn-group-justified" role="group" aria-label="id_art${item.id}">
	      <a id="id_art${positionRef.baseId}_${entity.id}" class="btn btn-default btn-xs"  role="button" data-toggle="collapse" href="#id_art_content${positionRef.baseId}_${entity.id}" aria-expanded="true" aria-controls="id_art_content${positionRef.baseId}_${entity.id}">
						 ...
						 <span class="glyphicon glyphicon-list" aria-hidden="false"></span>
						  ${entity.professionNo} -- ${getSubstring(entity.title,12,'无标题')} <span class="pull-right"> <strong>#${item?index}</strong></span>
      ...
      <div id="id_art_content${positionRef.baseId}_${entity.id}" class="panel-collapse collapse">
		  <div class="panel panel-default">
				  <div class="panel-body">
				    ...
				    <p><strong>子标题:</strong> ${entity.subTitle}</p>
					  <p>
				     	<strong>发布状态:</strong> 
				      	<#assign publishLabel=Profession.PUBLISH_STATUS.getLabel(entity.publishStatus)/>
				      	${publishLabel}
				      </p>
 ...
 <#macro M_professionPositionPageIngenuity positon=null entityPage=null flag="QX" tip="倾诚打造精品，坚守独道匠心">
...
<#if entityPage! && entityPage.totalElements gt 0 >
	...
	<div class="jumbotron">
	  ...
	  <a class="btn btn-danger btn-lg" href="/sz0099/ood/home/article/index/recommend?st=general" role="button">进入圈里</a>
	  </p>
	</div>
	 -->
	<@M_layout_jumbotron_single2 entity0=positon idPre="ly_index_fetch_ingenuity" tip=tip withPreIntro=true withTitle=true withSubTitle=true linkable=true/>
</#if>
</div>
</#macro>