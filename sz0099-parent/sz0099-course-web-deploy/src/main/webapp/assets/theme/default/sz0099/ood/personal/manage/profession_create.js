	...
	//标签操作开始
	function saveTag(professionId, url){
		
		var messageTipId="id_messageTip_tag";
		var name = $("#id_tag"+professionId).val();
  		var tagChecked = validateEmpty("id_tag"+professionId, "标签内容不能为空", messageTipId);
  		if(!tagChecked){
  			return false;
  		}
  		
  		var nameScriptChecked = validateScript("id_tag"+professionId, "标签含有非法字符，自动过滤", messageTipId);
  		if(!nameScriptChecked){
  			return false;
  		}
	...
	function deleteTag(id, professionId, tagId, deleteTagId, url){
		var messageTipId="id_messageTip_tag";
		$(deleteTagId).attr("disabled",true);
		
		if(typeof id == 'undefined' || typeof professionId == 'undefined' || typeof tagId == 'undefined'){
			alert("数据非法，请刷新重试！");
		}
	...
	function clearTag(professionId){
		$("#id_tag"+professionId).val('');
	}
	//标签操作结束
