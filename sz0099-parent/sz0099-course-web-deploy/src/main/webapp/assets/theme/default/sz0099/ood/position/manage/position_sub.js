...
function showRefWrapper(refWrapperId,collapseContentId, inputLoadedId,  msgTipId){
	...
	currentRefWrapperDialog=BootstrapDialog.show({
    	...
    	onshown:function(){
    		...
    		$("#divShowData span[id^='icon_id_btn_bind']").each(function(index,element){
    			console.log($(this).attr("id"));
    			var cur_id=$(this).attr("id");
    			$(this).attr("id",cur_id+"_");
    		})
...
function changeRefBtn(operate, id, viewType, baseId, positionId, mainId, bindBtnId, unbindBtnId,url){
	if(operate=="add"){
	    ...
	    $("#icon_"+unbindBtnId).removeClass("hidden");
	    $("#icon_"+bindBtnId).addClass("hidden");
	    ...
	    $("#icon_"+unbindBtnId+"_").removeClass("hidden");
	    $("#icon_"+bindBtnId+"_").addClass("hidden");
		...
		if(_spanIndex>0){
			...
			$("#icon_"+unBindBtn).addClass("hidden");
		}
		...
		$("#icon_"+unbindBtnId).addClass("hidden");
	    $("#icon_"+bindBtnId).removeClass("hidden");
		
		
		//修正弹框数据
	    $("#"+unbindBtnId+"_").addClass("hidden");
	    $("#"+bindBtnId+"_").removeClass("hidden");
	    $("#icon_"+unbindBtnId+"_").addClass("hidden");
	    $("#icon_"+bindBtnId+"_").removeClass("hidden");
 ...
 * @returns
 */
function syncPositionRef(id, viewType, baseId,positionId, mainId, spanId){
	var url=$("#"+spanId).attr("data-url");
	var tip="同步";
	var title=$("#id_p_title"+id).html();
	var refWrapper1 = "<p id='id_ref_tip'>确定要【"+tip+"】-"+title+"-关联吗？同步后不可回退！</p>";
	BootstrapDialog.show({
		title: "同步关联信息",
    	...
    	buttons: [{
            ...
            action: function(dialog){
            	...
            	var checked=true;
            	if(checked){
            		doSyncPositionRef(id, url, viewType, baseId,positionId, mainId,dialog);
            	}
            },
        }, {
            label: '取消',
...
function doSyncPositionRef(id,url, viewType, baseId, positionId, mainId,dialog){
	if(id && url){
		var uploadData={
				'id':id,
				'baseId':baseId,
