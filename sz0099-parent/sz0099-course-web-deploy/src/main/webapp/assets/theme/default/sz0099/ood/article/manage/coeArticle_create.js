	...
	function validateBaseinfo(articleId, messageDivId){
		...
		var messageTipBaseinfoId = messageDivId+articleId;
  		...
  		var descriptionLengthChecked = validateLength(descriptionId, "描述太长，都超255了", 255, messageTipBaseinfoId);
	...
	function syncTitle(self, articleId){
		var nameId="id_name"+articleId;
	...
	function validatePrice(articleId){
		...
		//title
  		var titleId="id_title"+articleId;
  		var titleChecked = validateEmpty(titleId, "标题不能为空", messageTipPriceId);
  		if(!titleChecked){
  			return false;
  		}
  		
  		var titleLengthChecked = validateLength(titleId, "标题太长，都超32了", 32, messageTipPriceId);
  		if(!titleLengthChecked){
  			return false;
  		}
  		
  		/*var titleScriptChecked = validateScript(titleId, "标题含有非法字符，自动过滤", messageTipPriceId);
	...
	//标签操作开始
	function saveTag(articleId, url){
		
		var messageTipId="id_messageTip_tag";
		var name = $("#id_tag"+articleId).val();
  		var tagChecked = validateEmpty("id_tag"+articleId, "标签内容不能为空", messageTipId);
  		if(!tagChecked){
  			return false;
  		}
  		
  		var nameScriptChecked = validateScript("id_tag"+articleId, "标签含有非法字符，自动过滤", messageTipId);
  		if(!nameScriptChecked){
  			return false;
  		}
    	...
    	.then(function (response) {
    	    var respMsg=$('<div></div>').html(response.data);
    	    $("#id_p_tags"+articleId).append(response.data);
	...
	function deleteTag(id, articleId, tagId, deleteTagId, url){
		var messageTipId="id_messageTip_tag";
		$(deleteTagId).attr("disabled",true);
		
		if(typeof id == 'undefined' || typeof articleId == 'undefined' || typeof tagId == 'undefined'){
			alert("数据非法，请刷新重试！");
		}
	...
	function clearTag(articleId){
		$("#id_tag"+articleId).val('');
	}
	//标签操作结束
	...
	function mergeForPublish(articleId, url){
		...
		var priceChecked = validatePrice(articleId);
		if(!priceChecked){
			return false;
		}
		var uploadData=getEntity(articleId);
